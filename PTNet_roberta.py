...
class PTNetRoBertaClassifier(nn.Module):
    def __init__(self, bert, opt):
        super().__init__()
        self.opt = opt
        bertmodel, bertmaskmodel = bert
        self.fine_model = BertModel(bertmodel, opt)
        self.pro_model = PROModel(bertmodel, opt)
...
class PROModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        labels = torch.where(input_ids == torch.tensor([50264], device='cuda:0'), label_ids, -100)
