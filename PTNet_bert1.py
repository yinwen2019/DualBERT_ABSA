...
class PTNetBert1Classifier(nn.Module):
    def __init__(self, bert, opt):
        super().__init__()
        self.opt = opt
        self.fine_model = BertModel(bert, opt)
...
class PROModel(nn.Module):
    def __init__(self, bert, opt):
        ...
        self.layernorm4bert = LayerNorm(opt.bert_dim)
        self.Wxx = nn.Linear(opt.bert_dim, self.attdim)

        self.dense = nn.Linear(opt.bert_dim, opt.bert_dim)
        self.decoder = nn.Linear(opt.bert_dim, self.vocab_size)

    def forward(self, inputs):
        input_ids, token_type_ids, attention_mask, label_ids, asp_start, asp_end, mask_index, aspect_mask = inputs
        outputs = self.bert(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids)
        sequence_output = outputs[0]# [16, 100, 768]
        sequence_output = self.dense(sequence_output)
        sequence_output = F.gelu(sequence_output)
        sequence_output = self.layernorm4bert(sequence_output)
        prediction_scores = self.decoder(sequence_output) # [16, 100, 768]
        loss_fct = torch.nn.CrossEntropyLoss()
        labels = torch.where(input_ids == torch.tensor([103], device='cuda:0'), label_ids, 0)
        masked_lm_loss = loss_fct(prediction_scores.view(-1, self.vocab_size), labels.view(-1))

        batch = mask_index.size(0)
        mask_index = mask_index.unsqueeze(-1).expand(batch, self.bert_dim).unsqueeze(-1)
        outputs = outputs.last_hidden_state.reshape(batch, self.bert_dim, 100)
        ...
        return outputs, masked_lm_loss / 10
